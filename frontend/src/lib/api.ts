...
export async function createCustomer(customer: Customer): Promise<ApiResponse<Customer>> {
  return await apiRequest<Customer>('/customers', 'POST', customer);
}

export async function updateCustomer(id: number, customer: Customer): Promise<ApiResponse<Customer>> {
...
export async function searchCustomers(query: string) {
  return api.get<Customer[]>(`/customers/search?q=${encodeURIComponent(query)}`).then((data)=>{
    return data?.data;
  });
}
